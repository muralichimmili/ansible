...
CREATE() {
  ...
  if [ $COUNT -eq 0 ]; then
  aws ec2 run-instances --launch-template LaunchTemplateId=lt-0dc3347829b90e6c4 --tag-specifications  "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
